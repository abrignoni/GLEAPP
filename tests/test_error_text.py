...
def test_scrub_local_paths_tidies_what_a_removed_path_leaves():
    """A path removed from inside brackets leaves ``()``, and one removed from before
    punctuation leaves a space in front of it; both go with the path."""
    assert imaging.scrub_local_paths("failed to open (/ev/x.zip): '/ev/x.zip'",
                                     "/ev/x.zip") == "failed to open"
    assert imaging.scrub_local_paths("cannot open /ev/x.zip : denied") == "cannot open: denied"
    assert imaging.scrub_local_paths("read 3 of 4 (partial), see log") == \
        "read 3 of 4 (partial), see log"
...
def test_process_one_stores_no_archive_path(tmp_path, monkeypatch, zip_path, leaks):
    ...
    assert payload["status"] == "error"
    text = payload["fields"]["error"]
    assert text == ("source archive unavailable: cannot open the source archive "
                    "([Errno 2] No such file or directory)"), text
    for leak in leaks:
