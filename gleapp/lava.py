...
def _artifact_vic(writer: "_Writer", rows: list[dict], media: dict[int, str]) -> None:
    ...
    writer.add_artifact(
        ...
        notes=(
            ...
            "Media ID, Device Path, Series, VIC Tags and the five flags are what that "
            "file stated, with each flag shown as yes or no: they are the importing "
            "organisation's record, not findings this tool made or checked. Category "
            "is the file's category in this case, which may have come from the VIC "
            "file, from a hash-set match or from the examiner. "
            "Media ID and Device Path are the identifiers the VIC file used, so a row "
            "can be matched back to it. Each of the five flags reads 'yes' or 'no' "
            "when the record carried it as a boolean, as the text true or false in any "
            "letter case, or as 1 or 0, and is blank otherwise, including when the "
            "record did not carry it; a blank is not a 'no'. Series and VIC Tags are "
            "the record's Series and Tags fields as stored (a list of tags one per "
            "line, and a series written as an object shown by its name or title "
            "field); what a "
            "series or tag stands for is not "
            "established here. Both are the importing organisation's, and are kept "
            "apart from the Flags column "
...
def _artifact_vic_hashset_matches(writer: "_Writer", rows: list[dict],
    ...
    writer.add_artifact(
        ...
        notes=(
            ...
            "in any letter case, or as 1 or 0, and is blank otherwise, including when "
            "the record did not carry it; a blank is not a 'no'. VIC Exif is the "
            "record's Exif rows, one property name and value per line in the order "
            "stored; a row with no property name is left out, and a PropertyGroup "
            "value, which the Project VIC 2.0 model allows, is not kept. The model "
            "keys each Exif row to its record's MD5, so the reading is the set's "
            "record of that file, not one this tool took; a "
            ...
            "ProjectVicDataModel.2.0.xml#L47-L56"))
...
_MEDIA_NOTES = (
    ...
    "that means depends on the filesystem. NTFS records instants, so Modified, "
    "Created and Accessed are filled from it. FAT and exFAT record a wall clock with "
    "no zone at all, so all three columns are empty for a FAT or exFAT volume and "
    "the readings appear instead under Recorded (as stored, no zone), exactly as "
    "the filesystem holds them and never converted: no instant can be derived from "
    "a reading whose zone is unknown, and exFAT's own stored UTC offset is shown as "
    "part of the reading rather than applied to it. FAT keeps only a date for the "
    "last access, so its reading carries an accessed date and no time of day. A "
    "file carved from unclaimed space has no timestamp of any kind, so for "
