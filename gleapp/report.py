 ...
 *{{box-sizing:border-box}}
 ...
 .muted{{color:var(--mut)}}
 footer.gen{{display:flex;align-items:center;gap:8px;margin-top:28px;padding-top:10px;
   border-top:1px solid var(--line);color:var(--mut);font-size:12px}}
 footer.gen img{{width:24px;height:24px}}
...
_LOGO = Path(__file__).parent / "web" / "static" / "gleapp-logo.png"


def _footer_html() -> str:
    """The GLEAPP logo and version, embedded so the report stays one standalone file."""
    from . import __version__
    try:
        uri = "data:image/png;base64," + base64.b64encode(_LOGO.read_bytes()).decode("ascii")
        img = f"<img src='{uri}' alt=''>"
    except OSError:
        img = ""
    return f"<footer class='gen'>{img}<span>Generated by GLEAPP {html.escape(__version__)}</span></footer>"
