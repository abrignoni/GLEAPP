...
The lint job runs pylint over the files a pull request touches. Long-lived modules
accumulate warnings that are deliberate rather than fixable, and a pull request that
happens to touch one of those files then goes red for reasons that have nothing to do
with the change. That pushes contributors toward either unrelated refactors or blanket
file-level `# pylint: disable` comments. Both are worse than the debt.
...
Two details matter for a stable comparison:
...
Usage:
    python tools/lint_changed.py --base-ref <sha> <file> [<file> ...]
"""
